...
public class TcpClientTestStepDefinitions {
	...
	public void fluxer_is_created_for_an_empty_string_host_at_the_found_free_port_number(String fluxer)
			throws Exception {
	...
	public void a_new_subscription_to_fluxer_status(String fluxer) {
		...
		fluxerToTest.state().doOnError(error -> {
			...
			if ("TCP client 1".equals(fluxer)) {
				tcpClientFluxer1StatusList.add(status.getValue());
			} else if ("TCP server 1".equals(fluxer)) {
				tcpServerFluxer1StatusList.add(status.getValue());
			}
		}).subscribe();
	}
	...
	// @Then("the subscriber of status of {string} receives: STOPPED -> STARTING ->
	...
	// public void tcp_client_1_writes_a_binary_message_to_the_tcp_server_1(String
	// messageToSend) {
	// final CountDownLatch countDownLatch1 = new CountDownLatch(1);
	//
	// Link targetLink = new Link(new Endpoint(HOST_TO_TEST, -1), new
	// Endpoint(HOST_TO_TEST, portNumber1),
	// Link.Status.NONE);
	//
	// tcpClientFluxer1.write(new Message(targetLink,
	// ByteBufUtil.decodeHexDump(messageToSend))).doOnError(ex -> {
	...
	// public void tcp_server_writes_a_binary_message_to_the_tcp_client(String
	// messageToSend) {
	// final CountDownLatch countDownLatch1 = new CountDownLatch(1);
	//
	// tcpServerFluxer1.write(new Message(connectedClientLink,
	// ByteBufUtil.decodeHexDump(messageToSend)))
	// .doOnError(ex -> {
	...
	// }
	//
	// @Then("TCP server 1 publishes its status changes: stopped -> starting ->
	// started -> stopping -> stopped")
	// public void
	// tcp_server_1_status_changes_stopped_starting_started_stopping_stopped() {
	...
	// }
	//
	// @Then("TCP server 1 publishes its read changes: 2 binary messages {string}
	// and {string}")
	// public void tcp_server_publishes_its_read_changes_2_binary_messages(String
	// expectedMessage1,
	...
	// public void tcp_server_publishes_its_read_changes_1_binary_message(String
	// expectedMessage1) {
	// StepVerifier.create(tcpServerFluxer1.read()).expectNextMatches(message -> {
	// return ByteBufUtil.hexDump(message.getMessage()).equals(expectedMessage1)
	// && message.getLink().getLocalEndpoint().getIpAddress().equals(HOST_TO_TEST)
